...
<div id="mySideFiltr" class="sidefilter">
		...
		<table>
				...
				echo"";
					$result=mysql_query("SELECT COUNT(id_establecimiento) as count FROM Establecimiento WHERE categoria LIKE '%$C%' AND (Establecimiento.zona LIKE '%$Loc%' OR Establecimiento.ciudad LIKE '%$Loc%')  AND (sub_categoria1 LIKE '$SC' OR sub_categoria2 LIKE '$SC' OR sub_categoria3 LIKE '$SC' OR sub_categoria1 LIKE '$SC') AND delivery LIKE '$D' AND price_range LIKE '$P' "); while($row = mysql_fetch_array($result)) 
					...
					$result = mysql_query("SELECT id_establecimiento, logo_url, LEFT(`nombre`,25) as nombre, categoria, price_range, CONCAT(LEFT(CONCAT(sub_categoria1, ', ',sub_categoria2,', ', sub_categoria3),32),'') as sub_categorias, telef_1, telef_2, LEFT(`direccion`,26) as direccion
					FROM Establecimiento, Rango_precio
					WHERE Rango_precio.signo = price_range AND categoria LIKE '%$C%' AND (Establecimiento.zona='$Loc' OR Establecimiento.ciudad='$Loc') AND (sub_categoria1 LIKE '$SC' OR sub_categoria2 LIKE '$SC' OR sub_categoria3 LIKE '$SC' OR sub_categoria1 LIKE '$SC') AND delivery LIKE '$D' AND price_range LIKE '$P'  ORDER BY special DESC, Nombre   ");
